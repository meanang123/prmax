...
class SearchContactController(EmbeddedBaseController):
	...
	def results(self, *args, **params):
		...
		contacts = session.query(Contact.contactid).\
		    filter(Contact.familyname.ilike(familyname)).\
		    filter(Contact.firstname.ilike(firstname)).\
		    filter(Contact.customerid == -1).all()
		
		employees = session.query(Employee.employeeid).\
		    filter(Employee.contactid.in_(contacts)).\
		    filter(Employee.customerid == -1).all()
		
		contacthistory = session.query(ContactHistory.contacthistoryid).\
		    filter(ContactHistory.ref_customerid == params['customerid']).\
		    filter(or_(ContactHistory.employeeid.in_(employees), ContactHistory.contactid.in_(contacts))).all()

		data = add_config_details({"familyname":familyname, "firstname":firstname, "contacthistory":39575})
	...
	def list(self, *args, **params):
		...
		for contacthistory in contacthistories:
			...
			newitem2 = {
		        ...
		        "subject%s" %row : contacthistory[1],
