...
# symbols which are imported by "from prmaxtouch.command import *"
...
from prmaxtouch.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("prmaxtouch"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="prmaxtouch.config")
...
def bootstrap():
    ...
    By default this runs the function 'prmaxtouch.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxtouch = prmaxtouch.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxtouch %s" % version)
    ...
    _read_config(args)
    from prmaxtouch.model import bootstrap_model
...
def start():
    ...
    from prmaxtouch.controllers import Root
