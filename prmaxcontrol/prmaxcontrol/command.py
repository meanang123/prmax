...
# symbols which are imported by "from prmaxcontrol.command import *"
...
from prmaxcontrol.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('prmaxcontrol'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='prmaxcontrol.config')
...
def bootstrap():
    ...
    By default this runs the function 'prmaxcontrol.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxcontrol = prmaxcontrol.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxcontrol %s" % version)
    ...
    _read_config(args)
    from prmaxcontrol.model import bootstrap_model
...
def start():
    ...
    from prmaxcontrol.controllers import Root
