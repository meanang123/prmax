...
# symbols which are imported by "from prmaxquestionnaires.command import *"
...
from prmaxquestionnaires.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('prmaxquestionnaires'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='prmaxquestionnaires.config')
...
def bootstrap():
    ...
    By default this runs the function 'prmaxquestionnaires.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxquestionnaires = prmaxquestionnaires.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxquestionnaires %s" % version)
    ...
    _read_config(args)
    from prmaxquestionnaires.model import bootstrap_model
...
def start():
    ...
    from prmaxquestionnaires.controllers import Root
