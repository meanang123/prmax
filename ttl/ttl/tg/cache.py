...
cache=load_backend(config.get('cache.backend','localmem://'))
...
def GetCache( key, current_version = 1 ):
    lock=threading.Lock()
    value,expiration,version=_get(key)
...
def UpdateCache( key = None, version = 1,
                 timeout_seconds = _decorator_timeout_seconds,
                 data = None, compress_ext = "html"):
    lock=threading.Lock()
    lock.acquire()
    try:
        if compress_ext.lower() in ("html", "xhtml"):
            data = slimmer.slimmer(data, compress_ext.lower())
        _set(key, data, timeout_seconds, version)
    finally:
        lock.release()


def prevent_cache( prmaxcontrol = ""):
    """ decorator to setup the repspose fields"""
    """ not used at the moment as the state_factory does this as well"""
    ...
    def decorator(func):
		...
		response.headers['Prmax'] = prmaxcontrol
