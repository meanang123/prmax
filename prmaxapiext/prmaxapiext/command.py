...
# symbols which are imported by "from prmaxapiext.command import *"
...
from prmaxapiext.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("prmaxapiext"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="prmaxapiext.config")
...
def bootstrap():
    ...
    By default this runs the function 'prmaxapiext.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxapiext = prmaxapiext.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxapiext %s" % version)
    ...
    _read_config(args)
    from prmaxapiext.model import bootstrap_model
...
def start():
    ...
    from prmaxapiext.controllers import Root
