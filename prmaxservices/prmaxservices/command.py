...
# symbols which are imported by "from prmaxservices.command import *"
...
from prmaxservices.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('prmaxservices'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='prmaxservices.config')
...
def bootstrap():
    ...
    By default this runs the function 'prmaxservices.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxservices = prmaxservices.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxservices %s" % version)
    ...
    _read_config(args)
    from prmaxservices.model import bootstrap_model
...
def start():
    ...
    from prmaxservices.controllers import Root
