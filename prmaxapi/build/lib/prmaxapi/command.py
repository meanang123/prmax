...
# symbols which are imported by "from prmaxapi.command import *"
...
from prmaxapi.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("prmaxapi"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="prmaxapi.config")
...
def bootstrap():
    ...
    By default this runs the function 'prmaxapi.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxapi = prmaxapi.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxapi %s" % version)
    ...
    _read_config(args)
    from prmaxapi.model import bootstrap_model
