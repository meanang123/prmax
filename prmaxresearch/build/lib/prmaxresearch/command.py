...
# symbols which are imported by "from prmaxresearch.command import *"
...
from prmaxresearch.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('prmaxresearch'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='prmaxresearch.config')
...
def bootstrap():
    ...
    By default this runs the function 'prmaxresearch.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-prmaxresearch = prmaxresearch.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prmaxresearch %s" % version)
    ...
    _read_config(args)
    from prmaxresearch.model import bootstrap_model
...
def start():
    ...
    from prmaxresearch.controllers import Root
