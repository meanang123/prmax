...
# symbols which are imported by "from prpublish.command import *"
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("prpublish"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="prpublish.config")
...
def bootstrap():
    ...
    By default this runs the function 'prpublish.model.bootstrap_model', which
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="prpublish %s" % version)
    ...
    _read_config(args)
    from prpublish.model import bootstrap_model
...
def start():
    ...
    from prpublish.controllers import Root
