...
from turbogears.finddata import find_package_data, standard_exclude_directories

import os

execfile(os.path.join("prpublish", "release.py"))

exclude_directories = ["dev", "sql", "tests"]+\
                    list(standard_exclude_directories)

packages = find_packages()
package_data = find_package_data(where='prpublish',
...
setup(
    ...
    entry_points={
        'console_scripts': [
            'start-prpublish = prpublish.command:start',
            # See the prpublish.command.bootstrap function for details
            'bootstrap-prpublish = prpublish.command:bootstrap',
        ],
    },
    cmdclass={
