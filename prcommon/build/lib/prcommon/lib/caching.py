...
def Invalidate_Cache_Object(plpy, SD, TD, objecttypeid ):
	...
	if SD.has_key("prmax_cache_outletcustomer_plan"):
		plan = SD['prmax_cache_outletcustomer_plan']
	else:
		plan = plpy.prepare("""DELETE FROM cache.cachestore
		WHERE objecttypeid = $1 AND objectid = $2""",
										["int", "int"])

	index = "new" if TD['event'] == "INSERT" else "old"
	if type( objecttypeid) == TupleType:
		for typeid in objecttypeid:
			plpy.execute(plan, [ typeid, TD[index][_getObjectIdKey(typeid)]])
	else:
		plpy.execute(plan, [ objecttypeid,
							 TD[index][_getObjectIdKey(objecttypeid)]])
