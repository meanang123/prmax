...
from prcommon.model.lookups import CustomerPaymentTypes
...
class PaymentSystemNew(object):
	...
	def encode_protex_form(params, customer):
		...
		cost = CustomerExternalNew.cost2(customer.customerid)
	...
	def protex_success(cls, params):
		...
		try:
			...
			protexinfo = PaymentSystemNew.decode_protex_form(params['crypt'])
		...
		try:
			...
			for(customerid,) in results:
				...
				if customer.next_month_value:
					...
					clippingcore = PaymentSystemNew.get_clipping_sub_value(customer, params["dd_collectiondate"])
					clippingfee = PaymentSystemNew.get_clippings_fee(customer, params["dd_collectiondate"])
	...
	def get_dd_invoice_run_params(cls, params):
		...
		query = PaymentSystemNew._DDCustomerList + \
...
#]
	...
	def dd_create_csv(cls, inparams):
		...
		try:
			...
			results = session.execute(text(PaymentSystemNew._DDCustomerCsv),
												        params,
												        PRMaxControl)
			for customerid in results:
...
class PrmaxCustomerInfoNew(BaseSql):
...
PrmaxCustomerInfoNew.mapping = Table('prmaxcustomerinfo', metadata, autoload=True, schema="internal")

mapper(CustomerExternalNew, CustomerExternalNew.mapping)
mapper(PrmaxCustomerInfoNew, PrmaxCustomerInfoNew.mapping)
