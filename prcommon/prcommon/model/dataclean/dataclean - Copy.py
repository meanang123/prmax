    ...
    def delete_customers(self, customers_todelete):
        ...
        for customerid in customers_todelete:
            ...
            try:
                ...
                #This is only when we want to completely delete a customer and his invoices/payments
                '''
                ...
                for customerinvoiceid in customerinvoices:
                    session.execute(text('DELETE FROM accounts.customerallocation WHERE alloc_invoiceid = :customerinvoiceid'), {'customerinvoiceid': customerinvoiceid}, CustomerInvoice)
                session.flush()    
                ...
                for customerpaymentid in customerpayments:
                    session.execute(text('DELETE FROM accounts.customerallocation WHERE alloc_paymentid = :customerpaymentid'), {'customerpaymentid': customerpaymentid}, CustomerPayments)
                    session.execute(text('DELETE FROM accounts.customerallocation WHERE source_paymentid = :customerpaymentid'), {'customerpaymentid': customerpaymentid}, CustomerPayments)
                session.flush()            
                ...
                for adjustmentid in customeradjustments:
                    session.execute(text('DELETE FROM accounts.customerallocation WHERE alloc_adjustmentid = :adjustmentid'), {'adjustmentid': adjustmentid}, Adjustments)
                    session.execute(text('DELETE FROM accounts.customerallocation WHERE source_adjustmentid = :adjustmentid'), {'adjustmentid': adjustmentid}, Adjustments)
                session.flush()
                    
                session.execute(text('DELETE FROM accounts.customerinvoices WHERE customerid = :customerid'), {'customerid': customerid}, CustomerInvoice)
                session.flush()
                session.execute(text('DELETE FROM accounts.audittrail WHERE customerid = :customerid'), {'customerid': customerid}, Adjustments)
                session.flush()
                

                session.execute(text('DELETE FROM accounts.adjustments WHERE customerid = :customerid'), {'customerid': customerid}, Adjustments)
                session.execute(text('DELETE FROM accounts.customerpayments WHERE customerid = :customerid'), {'customerid': customerid}, CustomerPayments)
                '''
