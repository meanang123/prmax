...
class Outlet(BaseSql):
	...
	def do_outlet_coverage(cls, outlet, activity, params ):
		...
		for outletcoverage in dbcoverage:
			...
			if not outletcoverage.geographicalid in coverage:
				ActivityDetails.AddDelete ( outletcoverage.geographicalid , activity.activityid , Constants.Field_Outlet_Coverage )
		...
		for geographicalid in coverage:
			if not geographicalid in dbcoverage2:
				geog = OutletCoverage(
						outletid = outlet.outletid,
				    geographicalid = geographicalid)
				ActivityDetails.AddAdd ( geog.geographicalid , activity.activityid , Constants.Field_Outlet_Coverage )
				session.add(geog)
	...
	def research_update_prn(cls, params):
		...
		try:
			...
			ActivityDetails.AddChange ( outlet.prmax_outlettypeid , params['prmax_outlettypeid'] ,
			                            activity.activityid ,
			                            Constants.Field_Outlet_Type )
	...
	def research_set_primary(cls, params):
		...
		try:
			...
			if outlet.primaryemployeeid !=  employee.employeeid :
				...
				session.add ( activity )
				session.flush()
				ActivityDetails.AddChange ( outlet.primaryemployeeid ,  employee.employeeid , activity.activityid, Constants.Field_Primary_Contact)
	...
	def research_update(cls, params):
		...
		try:
			...
			if int(params["contactid"]) != employee.contactid:
				...
				ActivityDetails.AddChange(employee.contactid, params["contactid"], activity.activityid, Constants.Field_Contactid)
				# delete this contact
				index_fields_employeeid = (
					(Constants.freelance_employeeid, 'familyname', StandardIndexerInternal.standardise_string, None),
					(Constants.mp_employeeid, 'familyname', StandardIndexerInternal.standardise_string, None),
