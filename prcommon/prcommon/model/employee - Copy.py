...
class EmployeeDisplay(BaseSql):
	...
	def _GetEmployeeDisplay(cls, params):
		...
		if details['twitter'] != None and details['twitter'] != '':
			from prcommon.model.twitteruserdetails import TwitterUserDetails
			twd = TwitterUserDetails.query.get(params['employeeid'])
			if twd and twd.profile_image_url:
				profile_image_url = twd.profile_image_url
			elif not twd:
				twd = TwitterUserDetails(
					objectid=params['employeeid'], 
					objecttypeid=Constants.Cache_Display_Employee, 
					twitterlink=details['twitter'], 
					statusid=1)
				session.add(twd)
				CacheControl.Invalidate_Cache_Object_Research(params['employeeid'], Constants.Cache_Display_Employee)
