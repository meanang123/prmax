...
class ConnectToImap(object):
    ...
    _Email_Support_User = "stamatia.vatsi@prmax.co.uk"
    _Email_Support_Password = "!!!gmatoyla"
    ...
    def archiveMessage(self, msg_id, dest_folder):
        """ move a message to the archive mail store """
        self._connection.store(msg_id, '+X-GM-LABELS', dest_folder)
...
class AnalysisMessages(ConnectToImap, DBConnect):
    ...
    def run(self):
        ...
        for i, msg_id in enumerate(self._msg_ids):
            ...
            rep = self.getMessage(msg_id)
            if rep == msg_id:
                emaillist.append(msg_id)
                
        researchers = session.query(User.display_name).filter(User.isuserresearcher == True).all()
        researchersnbr = len(researchers)
        i = 0
        for email in emaillist:
            self.archiveMessage(email, str(researchers[i][0]))
            i += 1
            if i == researchersnbr:
                i = 0
